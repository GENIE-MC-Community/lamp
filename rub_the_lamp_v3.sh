...
CHECKOUT="GITHUB"  # Alternate option is "GITHUB"
TAG="R-3_00_02"       # SVN Branch
...
USERREPO="GENIE-MC"      # "USER REPO" == just User, really
GENIEVER="Generator"        # "VER" == repo name (really)
GITBRANCH="master"    # 
...
if [[ $SUPPORTTAG == "head" || $SUPPORTTAG == "HEAD" ]]; then
    ...
    echo "Switching to tag $SUPPORTTAG ..."
    git checkout $SUPPORTTAG
fi
DEBUGFLAG=""
if [ "$DEBUG" == "yes" ]; then
    DEBUGFLAG="--debug"
