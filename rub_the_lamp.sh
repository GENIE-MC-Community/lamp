...
./configure --enable-debug --enable-test --enable-numi --enable-t2k --enable-atmo --enable-rwgt --enable-validation-tools --with-optimiz-level=O0 --with-log4cpp-inc=$LOG4CPP_INC --with-log4cpp-lib=$LOG4CPP_LIB >& log.config
echo "Building GENIE..."
$MAKE >& log.make
if [ $? -eq 0 ]; then
